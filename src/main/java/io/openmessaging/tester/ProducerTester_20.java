...
public class ProducerTester_20 {

    static Logger logger = LoggerFactory.getLogger(ProducerTester_20.class);
    //0表示默认;
            ...
            while (true) {
                try {
                    ...
                    if (sendNum >= Constants.PRO_MAX) {
                        System.out.println("producing is over");
                        producer.flush();
                        break;
                    }
                } catch (Exception e) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int sendNum = 0;
        for (int i = 0; i < ts.length; i++) {
            sendNum += ((ProducerTask)ts[i]).getSendNum();
        }
        long end = System.currentTimeMillis();
        logger.info("Produce Finished, Cost {} ms, Num {} ", end - start, sendNum);
        System.out.println("" + (end - start) + "  " + sendNum);
    }
}
